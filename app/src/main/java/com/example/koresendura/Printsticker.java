...
public class Printsticker extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_printsticker);
        ...
        barcodeinput.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                datePickerDialog = new DatePickerDialog(Printsticker.this,
                                                        new DatePickerDialog.OnDateSetListener() {
                                                            @Override
                                                            public void onDateSet(DatePicker datePicker, int year, int month, int day) {
                                                                barcodeinput.setText(day + "/" + (month + 1) + "/" + year);
                                                            }
                                                        }, year, month, dayOfMonth);
                calendarmin.add(Calendar.DATE, -6);
        ...
        backbtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent myIntent = new Intent(Printsticker.this, selectsize.class);
                Printsticker.this.startActivity(myIntent);
            }
        });
...
//				        		HPRTPrinter=null;
    ...
    public void print(View view)
    {
        ...
        try
        {
            ...
            if(inputlabelsize.length()==0||inputtype.length()==0||inputcore.length()==0||inputbarcode.length()==0||inputcopy.length()==0)
            {
                ...
                if (labeltype.getText().toString().equals("75x50")) {
                    String prnfile = prnFile.l_75x50(inputlabelsize, inputtype, inputcore, inputbarcode, inputcopy);
                     //Toast.makeText(getApplicationContext(),"75*50",Toast.LENGTH_LONG).show();
                    hprtPrinterHelper.printData(prnfile);
                    hprtPrinterHelper.end();
                    prnfile="";

                } else if (labeltype.getText().toString().equals("100x20")) {
                    String prnfile2 = prnFile.l_100x20(inputlabelsize, inputtype, inputcore, inputbarcode, inputcopy);
                    Toast.makeText(getApplicationContext(),"100x20",Toast.LENGTH_LONG).show();
                    // Toast.makeText(getApplicationContext(),prnfile,Toast.LENGTH_LONG).show();
                    hprtPrinterHelper.printData(prnfile2);
                    prnfile2="";
                    hprtPrinterHelper.end();
                }else if(labeltype.getText().toString().equals("75x50box")){
                    String prnfile3 = prnFile.boxl_75x50(inputlabelsize, inputtype, inputcore, inputbarcode, inputcopy);
...
//				HPRTPrinter=new ZPLPrinterHelper(thisCon,arrPrinterList.getItem(spnPrinterList.getSelectedItemPosition()).toString());
    ...
    public void onScan(View view){
        Intent myIntent = new Intent(Printsticker.this, Scanner.class);
        Printsticker.this.startActivity(myIntent);
